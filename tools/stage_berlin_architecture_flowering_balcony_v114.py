...
BASE=os.path.join(STAGE,'refined-models');STAGE=os.path.join(STAGE,'flowering-balcony');OUT=os.path.join(STAGE,'models')
...
def leaf(center,direction,width,length,color,tilt):
    ...
    count=12
    points=[center+direction*(math.cos(i*math.tau/count)*length)+side*(math.sin(i*math.tau/count)*width) for i in range(count)]
    normals=[(normal*.70+(p-center).normalized()*.30).normalized() for p in points]
    points.extend([center+normal*tilt,center-normal*tilt*.35]);normals.extend([normal,-normal])
    points.extend(points[:count]);normals.extend([-n for n in normals[:count]])
    faces=[]
    for i in range(count):faces.extend([(count,i,(i+1)%count),(count+1,count+2+(i+1)%count,count+2+i)])
    form.surface(points,orient(points,faces,normals),normals,color,label='rounded trailing balcony leaf')
...
def bloom(center,normal,size,color):
    ...
    points=[];normals=[]
    for i in range(30):
        a=2*math.pi*i/30;r=size*(.79+.21*math.cos(5*a));p=center+right*(math.cos(a)*r)+up*(math.sin(a)*r)
    ...
    points.extend(points[:30]);normals.extend([-n for n in normals[:30]])
    for i in range(30):faces.extend([(30,i,(i+1)%30),(31,32+(i+1)%30,32+i)])
...
for cluster in range(9):
    angle=LOW+.07+(HIGH-LOW-.14)*cluster/8;center=at(angle,2.57,9.85)
    ...
    for j in range(9):
        ...
        if j>=6:direction=(Vector((0,-1,0))+radial*.50+tangent*math.cos(a)*.35).normalized()
        leaf(center+spread+Vector((0,(j%3)*.055 if j<6 else -.15-(j-6)*.13,0)),direction,.105+rng.random()*.05,.19+rng.random()*.09,[0x637e43,0x81974f,0x4e713e][j%3],.034)
    ...
    bloom(center+radial*.20-tangent*.07+Vector((0,-.09,0)),radial+Vector((0,.18,0)),.113,[0xf1c9ad,0xe5a99a,0xdd9c95][cluster%3])
    bloom(center+radial*.13+tangent*.20+Vector((0,-.18,0)),radial+Vector((0,.1,0)),.084,[0xe5a99a,0xdd9c95,0xf1c9ad][cluster%3])

addition=form.native();addition.location=obj.location.copy();addon=packed_geometry(addition)
assert addon['triangles']<6500,addon['triangles']
...
after['plantingRevision']='rounded-trailing-flowers-v114';after['balconyRevision']='sculpted-curved-balcony-v114';after['balconyBaseTriangles']=original['triangles'];after['balconyBaseVertices']=off
protected={}
...
updated['source']='Blender 5.2 / tools/stage_berlin_architecture_flowering_balcony_v114.py'
