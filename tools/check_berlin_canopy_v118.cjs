...
const root=path.resolve(__dirname,'..'),stage=path.join(root,'audit/berlin-parity-trees-v118'),name='berlin-kiez-garden-v1';
...
assert.equal(data.canopyRevision,'dense-layered-linden-v118');
...
for(const role of ['treeNear','treeStreetFar']){
  ...
  assert.ok(r.triangles<before.meshes[role].triangles,'both LODs reduce submitted geometry');
  ...
  assert.ok(bytes<750000,'shared geometry stays below 750 KB');
  report[role]={triangles:r.triangles,vertices:r.vertices,bytes,radius,minFacing};
}
// Rasterize orthographic silhouettes independently of the renderer: increased
// projected foliage must be real geometry in all four street-facing azimuths.
...
report.coverage=[0,Math.PI/4,Math.PI/2,Math.PI*3/4].map(angle=>{
  ...
  assert.ok(near>old*1.35,'larger crowns create significantly more projected foliage');
  assert.ok(far/near>.90&&far/near<1.03,'far silhouette preserves the close crown');return{angle,before:old,near,far,gain:near/old};
});
