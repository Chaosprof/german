"""Compose the V152 bus atlas and write the shipping bus data.

python tools/finish_berlin_bus_v152.py        (Python 3.11 with numpy + PIL)

Reads audit/berlin-vehicles-v152/bus/build/{parts.json,bake_body_*.npy} from
tools/stage_berlin_bus_v152.py (Blender) and writes
audit/berlin-vehicles-v152/bus/models/berlin-reference-bus-v152.{json,inline.js}.
...
PREFIX = 'berlin-reference-bus-v152'
...
# A kerbside bus reads from metres away: ship the 2048 bake at 1024.
alb_img = alb_img.resize((1024, 1024), Image.LANCZOS)
orm_img = orm_img.resize((512, 512), Image.LANCZOS)
...
data = {
    'source': 'Blender 5.2 / tools/stage_berlin_bus_v152.py + tools/finish_berlin_bus_v152.py',
    'finishRevision': 'reference-sd202-v152',
    'parkingContract': {'halfLength': 5.5, 'maxHalfWidth': 1.45},
...
(OUT / (PREFIX + '.inline.js')).write_text('var BERLIN_REFERENCE_BUS_DATA = ' + raw + ';\n')
