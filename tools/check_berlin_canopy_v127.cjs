...
const root=path.resolve(__dirname,'..'),stage=path.join(root,'audit/berlin-parity-v127'),name='berlin-kiez-garden-v1';
...
assert.equal(data.canopyRevision,'fine-clustered-linden-v127');
...
for(const role of ['treeNear','treeStreetFar']){
  ...
  assert.ok(bytes<750000,'shared geometry retains the existing 750 KB budget');
...
report.coverage=[0,Math.PI/4,Math.PI/2,Math.PI*3/4].map(angle=>{
  ...
  assert.ok(near>old*.85&&near<old*.95,'fine leaves retain crown coverage without a solid core');
