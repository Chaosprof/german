...
function applyBerlinFacadeFinishShader(THREE, material) {
  ...
  applyBerlinFacadeFinishShader(THREE,material);
  ...
  function build(w, variant, side) {
    ...
    if (typeof BERLIN_REFERENCE_ARCHITECTURE !== 'undefined' &&
      ...
      if (record) {
        ...
        authored=finishBerlinKiezFacade(THREE,authored,variant,w,side);
    ...
    for(var col=0;col<bays;col++) {
      ...
      if((col+variant)%2===0 || (variant===4 && col===1)) {
        box(xx,h+1.10,-.05,variant===4?1.57:1.31,1.31,1.22,body,1);
