...
const root=path.resolve(__dirname,'..'),stage=path.join(root,'audit/berlin-parity-v129'),name='berlin-kiez-garden-v1';
...
assert.equal(data.canopyRevision,'fine-oval-linden-v129');
...
for(const role of ['treeNear','treeStreetFar']){
  ...
  assert.ok(r.triangles<originalBudget.meshes[role].triangles,'both LODs stay below the original V126 geometry budget');
