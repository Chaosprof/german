...
for (const side of [-1, 1]) {
  ...
  for (let i = 1; i < street.length; i++) {
    ...
    assert.ok(Math.abs(gap-2/3)<.001||Math.abs(gap-24)<.001,
...
const projection = [];
for (const [slot, cell, expectedLeft, expectedRight] of [[5,5,7,25], [2,4,75,99]]) {
  ...
  assert.ok(left >= expectedLeft && right <= expectedRight, 'actual authored goods are inside the requested outer-third frame');
  ...
  for (const x of sourceXs) for (const y of [1.25,2.05,2.8]) {
    ...
    if (hit && hit.object === mesh && atlasCell(hit.uv) === cell) visibleGoods++;
  }
  assert.ok(visibleGoods >= 10, 'perspective rays reach actual interiors without neighbor or own-portal occlusion');
  projection.push({shop: cell === 5 ? 'bakery' : 'bookstore', left, right, top, bottom, visibleGoods});
}
...
const props = {ud: {props: propSlots, buildings: opening.buildings}, referenceBeat: true, contactCount: 0,
...
for (const g of propSlots.map(slot => slot.variants[0])) {
  ...
  assert.ok(bounds.max.y-bounds.min.y>=1.48&&bounds.max.y-bounds.min.y<=1.52,
    'the full authored planter is1.50m tall after its child and root scales');
...
for(const [slot,xs]of [[5,[-4.2,-2.7,-1.2]],[2,[-4.2,0,4.2]]]) {
  ...
  for(const x of xs)for(const y of [1.9,2.6]) {
    ...
    const hit=ray.intersectObjects([...opening.buildings.filter(b=>b.visible),...propSlots.map(slot=>slot.variants[0])],true)[0];
    assert.ok(hit&&hit.object.parent===building,'foreground plants leave the actual display centres visible');
    planterPortalRays++;
  }
}
...
pooledPlanters.forEach(g=>fixtureChunk.add(g));
fixtureChunk.userData={lampBatches:[],treeBatches:[],manholes:[],lamps:[],props:pooledPlanters.map(g=>({variants:[g]}))};
...
assert.equal(poolContext.streetFixturePools.pools.length,1,'all actual planter meshes share one existing opaque furniture pool');
assert.equal(poolContext.streetFixturePools.sources.length,3);
