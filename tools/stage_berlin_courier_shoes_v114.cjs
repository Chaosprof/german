...
const root=path.resolve(__dirname,'..'),stage=path.join(root,'audit/berlin-model-forms-v114/courier/shoe-forms');fs.mkdirSync(stage,{recursive:true});
...
if(!fs.existsSync(path.join(__dirname,'berlin_courier_shoes_v114.js'))){console.log(JSON.stringify(info,null,2));process.exit(0);}
const sculpt=require('./berlin_courier_shoes_v114.js').sculptCourierShoesV114;
...
const patch={version:114,vertices:changed.attributes.position.count,count:ids.length,baseProbes:probes.flatMap(i=>[i,...vec(v97,i).toArray(),...vec(v97,i,'normal').toArray()]),data:payload.toString('base64')};
...
const report={stagedOnly:true,sourceHash:crypto.createHash('sha256').update(glb).digest('hex'),shoeInfo:info,changedShoePositions:shoeChanged.length,fullPatchVertices:ids.length,payloadBytes:payload.length,protectedNonShoeVertices:protectedIds.length,nonShoePositionsNormalsExact:true,UVSkinMasksTopologyAnimationsExact:true,bagBoundsExact:true,maxMove,minArea,maxArea,minFaceCos,minAlignment,maxSeamP,maxSeamN,poseSamples,maxPosedMove,maxFloorLowering,sculpt:changed.userData.courierShoesV114,packedEqualsSculpt:true,extraDraws:0,extraVertices:0,extraPerFrameWork:0};
