...
STAGE = ROOT / 'audit/berlin-parity-v129'
...
for li,(center,radii,count) in enumerate(lobes):
    ...
    for j in range(count):
        ...
        length=(.166+.066*rng.random())*(1-.12*max(0,normal[1]))
...
near_obj.data=detailed_mesh('Fine clustered linden crown v129',near_specs,material)
far_obj.data=detailed_mesh('Fine clustered linden crown far v129',far_specs,material)
near_obj['form_source']=far_obj['form_source']='tools/stage_berlin_canopy_v129.py'
...
assert near_record['triangles']<=original_budget['meshes']['treeNear']['triangles'] and far_record['triangles']<=original_budget['meshes']['treeStreetFar']['triangles']
...
updated['canopyRevision']='fine-oval-linden-v129'
updated['source']='Blender 5.2 / tools/stage_berlin_canopy_v129.py'
...
print('CANOPY_V129_COMPLETE',json.dumps(report),flush=True)
