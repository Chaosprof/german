...
STAGE = ROOT / 'audit/berlin-parity-v127'
...
def leaf(center, length, width, axis, roll, color, lod=False):
    """Closed, thin six-corner leaf; distance uses a closed tetrahedron.

    Alternating raised corners form an eight-triangle antiprism. Every edge
    ...
    xy=[(1,0),(-.6,1.05),(-.6,-1.05),(0,0)] if lod else [(1,0),(.40,.87),(-.52,.78),(-1,0),(-.52,-.78),(.40,-.87)]
    heights=[.055,.055,.055,-.09] if lod else [.07,-.07,.07,-.07,.07,-.07]
    verts=[center+u*x*length+v*y*width+n*width*heights[j] for j,(x,y) in enumerate(xy)]
    topology=[(0,1,2),(0,3,1),(1,3,2),(2,3,0)] if lod else [(0,2,4),(1,5,3),(0,1,2),(2,3,4),(4,5,0),(1,0,5),(3,2,1),(5,4,3)]
    faces=[]
    ...
    for fi,f in enumerate(faces):
        ...
        normals.append([native(unit(n*sign+u*xy[j%side_count][0]*.22+v*xy[j%side_count][1]*.38)) for j in f])
...
for li,(center,radii,count) in enumerate(lobes):
    ...
    for j in range(count):
        ...
        length=(.144+.057*rng.random())*(1-.12*max(0,normal[1]))
        ...
        shade=np.array([10,27,6]);light=np.array([104,139,23])
        ...
        lod_candidates.append((near_shape,leaf(p,length*1.55,width*1.80,axis,roll,color,True),j%2==0))
...
near_obj.data=detailed_mesh('Fine clustered linden crown v127',near_specs,material)
far_obj.data=detailed_mesh('Fine clustered linden crown far v127',far_specs,material)
near_obj['form_source']=far_obj['form_source']='tools/stage_berlin_canopy_v127.py'
...
assert near_record['triangles']<=baseline['meshes']['treeNear']['triangles'] and far_record['triangles']<=baseline['meshes']['treeStreetFar']['triangles']
...
updated['canopyRevision']='fine-clustered-linden-v127'
updated['source']='Blender 5.2 / tools/stage_berlin_canopy_v127.py'
...
print('CANOPY_V127_COMPLETE',json.dumps(report),flush=True)
