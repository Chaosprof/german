...
const source=read('source-check.json'),native=read('native-check.json'),visual=read('visual-check.json'),verification=read('verification.json'),plan=read('benchmark-plan.json'),r=read('benchmark/comparison.json');
for(const check of [source,visual]){assert.equal(check.passed,true);assert.equal(check.candidateSha256,sha(page));}
assert.equal(native.passed,true);assert.equal(verification.candidateSha256,sha(page));assert.equal(plan.candidateSha256,sha(page));
assert.equal(verification.performanceFixtureLogSha256,sha(fs.readFileSync(path.join(stage,'performance-check.log'))));
assert.ok(fs.readFileSync(path.join(stage,'performance-check.log'),'utf8').includes('PASS: three actual seven-tree chunks'));
...
if(process.argv.includes('--install')){
 ...
 for(const [name,hash]of Object.entries(source.addedAssets)){
  assert.equal(sha(fs.readFileSync(path.join(stage,'models',name))),hash,name);
  if(fs.existsSync(path.join(root,'assets/models',name)))assert.equal(sha(fs.readFileSync(path.join(root,'assets/models',name))),hash,'do not overwrite an unrelated asset');
 }
 let worker=fs.readFileSync(path.join(root,'sw.js'),'utf8');assert.ok(worker.includes('"artikel-blitz-v145"'));worker=worker.replace('"artikel-blitz-v145"','"artikel-blitz-v146"');
 for(const name of Object.keys(source.addedAssets))fs.copyFileSync(path.join(stage,'models',name),path.join(root,'assets/models',name));
 const temp=path.join(root,'berlin-runner.v146.tmp.html');fs.writeFileSync(temp,page);fs.renameSync(temp,path.join(root,'berlin-runner.html'));fs.writeFileSync(path.join(root,'sw.js'),worker);
 assert.equal(sha(fs.readFileSync(path.join(root,'berlin-runner.html'))),sha(page));
 for(const [name,hash]of Object.entries(source.addedAssets))assert.equal(sha(fs.readFileSync(path.join(root,'assets/models',name))),hash);
 for(const [file,hash]of Object.entries(source.protectedFiles))assert.equal(sha(fs.readFileSync(path.join(root,file))),hash);
 fs.writeFileSync(path.join(stage,'shipping.json'),JSON.stringify({pageSha256:sha(page),pageBytes:page.length,cacheRevision:'artikel-blitz-v146',protectedFiles:source.protectedFiles,addedAssets:source.addedAssets},null,2));
 console.log('Installed and verified V146 page, four native/packed container assets and cache revision.');
}
