...
const formsStart=html.indexOf('  // BEGIN COURIER_FORMS_PATCH_V114');
const formsEnd=html.indexOf('  // END COURIER_FORMS_PATCH_V114',formsStart);
const formsScope=formsStart<0?'':html.slice(formsStart,formsEnd);
...
if(formsScope)assert.equal(mesh.geometry.userData.courierFormsV114?.baked,true,'actual v14 applies the new rounded cuff/fold sculpt after the accepted cloth');
