...
assert.equal(geometry.layouts.length,36);assert.equal(geometry.addedDraws,0);assert.equal(geometry.addedShaders,0);assert.equal(geometry.addedTextures,0);
...
if(process.argv.includes('--install')){
 ...
 let worker=fs.readFileSync(path.join(root,'sw.js'),'utf8');assert.ok(worker.includes('"artikel-blitz-v146"'));worker=worker.replace('"artikel-blitz-v146"','"artikel-blitz-v147"');
 for(const name of Object.keys(geometry.sources))fs.copyFileSync(path.join(stage,'sources',name),path.join(root,'tools',name));
 const temp=path.join(root,'berlin-runner.v147.tmp.html');fs.writeFileSync(temp,page);fs.renameSync(temp,path.join(root,'berlin-runner.html'));fs.writeFileSync(path.join(root,'sw.js'),worker);
 ...
 fs.writeFileSync(path.join(stage,'shipping.json'),JSON.stringify({pageSha256:sha(page),pageBytes:page.length,cacheRevision:'artikel-blitz-v147',protectedFiles:geometry.protectedFiles,sources:geometry.sources},null,2));
 console.log('Installed and verified V147 page, two matching facade sources and cache revision.');
}
